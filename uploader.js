(function ($) {
	defaults = {
        ...
        placeholderParent: "fileimg",
		loadingCss: "file-preview-loading",
		tableTemplate: function () {
			return "<div class='files-uploads file-preview-table' id='file-preview-table'>" +
				"<div class='main_container'></div>" +
				"</div>";
		},
		rowTemplate: function (options) {
			return "<div class='" + config.tableRowClass + "'>" +
				"<div class='" + options.placeholderParentCss + "'>" + 
				"<img src='" + options.src + "' class='" + config.placeholderClass + "' />" + "</div>" +
				"<div class='file-details'>" +
				"<div class='filename'>" + options.name + "</div>" +
				"<div class='filesize'>" + options.size + "</div>" +
				"</div>" +
				"<div class='file-edit'>" +
                "<div class='filedownload'>" + "<a href='" + options.src + "' class='downlaoded_files'>Download</a>" + "</div>" +
				"<div class='remove-file'><span class='text-danger'>Remove</span></div>" +
				"</div>" +
				"</div>";
		},
	...
	var getFileTypeCssClass = function (filetype) {
		...
		return defaults.placeholderParent + " " + fileTypeCssClass;
	};
	...
	$.fn.uploadPreviewer = function (options, callback) {
		...
		if (window.File && window.FileReader && window.FileList && window.Blob) {
			...
			this.wrap("<span class='file_upload_btn " + config.shadowClass + "'></span>");
			buttonText = this.parent("." + config.shadowClass);
			buttonText.prepend("<span class='file_upload_icon'>" + config.button_text + "</span>");
			buttonText.wrap("<span class='" + config.buttonClass + "'></span>");

			previewTableIdentifier = options.preview_table;
			if (!previewTableIdentifier) {
				var getid = config.preview_box;
				//console.log(getid);
				$("span." + config.buttonClass).after(config.tableTemplate());
				var previewTableParentId = $(getid).find("div.files-uploads." + config.tableCss)
				previewTableIdentifier = previewTableParentId;
			}

			previewTable = $(previewTableIdentifier);
			previewTable.addClass(config.tableCss);
			previewTableBody = previewTable.find(".main_container");
			...
			this.on('change', function (e) {
				...
				$.each(e.currentTarget.files, function (index, file) {
					...
					reader.onload = function (fileReaderEvent) {
						var filesize, filetype, imagePreviewRow, placeholderParentCss, source;
						if (previewTableBody) {
							filetype = file.type;
							if (/image/.test(filetype)) {
								source = fileReaderEvent.target.result;
								placeholderParentCss = config.placeholderParent + " image";
							} else {
								source = "";
								placeholderParentCss = getFileTypeCssClass(filetype);
							}
							filesize = getFileSize(file.size);
							imagePreviewRow = previewRowTemplate({
								src: source,
								name: file.name,
								placeholderParentCss: placeholderParentCss,
								size: filesize
							});
